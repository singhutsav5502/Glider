...
| Transparent interception — cursor-agent's own traffic still isn't caught | **Re-tested live 2026-07-28, real negative result, not yet root-caused.** `claude -p` is confirmed working end-to-end through Glider's own transparent redirector. `cursor-agent -p ... --trust` (no proxy env vars, real WinDivert redirect only) was retested the same way and **none** of its traffic reached Glider at all — not the completion call, not even its early account/dashboard calls (`api2.cursor.sh`), despite `agentn.global.api5.cursor.sh` being covered by the `*.api5.cursor.sh` host pattern and `node.exe` (cursor-agent's real owning process — `cursor-agent.cmd` → `cursor-agent.ps1` → `node.exe`, confirmed by reading the wrapper scripts) already being in `allow_process_names`. Ruled out: WinDivert queue drops (`queue_full` counter was flat across the whole test window, not incrementing) and host-pattern mismatch (config and `TestHostMatcher` both confirm the wildcard matches). Not yet checked: whether `node.exe` traffic from *any* source ever gets redirected on this machine at all (would distinguish "cursor-agent specifically" from "node.exe process-name matching generally"), and whether IPv6 or a specific local network binding cursor-agent's Node runtime uses falls outside the WinDivert filter's scope. Deliberately not debugged further live this pass — the only running instance at the time was also carrying this session's own real traffic, and further poking (rebuilds, restarts) risked disrupting that. Needs a dedicated pass with a disposable instance. |
