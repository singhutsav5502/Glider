...
| Delegate replies over cursor-agent's own protocol timing out on a slow delegate target — **fixed 2026-07-29** | Root cause (confirmed live): `DelegateHandler` used to compute the full reply text via `vendors.ResolveDelegate` (runs the target vendor's CLI headless — for `agy`, can take many seconds, up to `vendors.RunTimeout`=120s) *before* ever touching the response writer, so cursor-agent's own HTTP/2 client received zero bytes for the whole delegate run and gave up (`http2: stream closed`). Fixed by making the whole `OriginAdapter.WriteReply` flow async: `WriteReply` now takes `(header string, replyText <-chan string)` instead of a resolved string; `DelegateHandler` runs `ResolveDelegate` in a goroutine and hands `WriteReply` a channel it fills once done. `header` (e.g. `"Delegated to agy:\n\n"`) is written immediately, before the slow call even starts — this both keeps the connection alive and tells the human what was actually delegated to whom, since a synthesized reply otherwise gives no indication a different CLI produced it. `cursorOriginAdapter.WriteReply` uses a new `cursorrpc.WriteDelegateReplyWithKeepAlive`, which also sends an extra heartbeat frame every 10s while waiting — the vendor with a confirmed client-side idle-stream timeout; `claude`/`agy` get the header-first treatment too (free correctness improvement) but no periodic ticker, since no timeout has been confirmed for either of their clients. Regression tests: `cursorrpc.TestWriteDelegateReplyWithKeepAlive_HeaderArrivesBeforeResultResolves`/`..._SendsPeriodicHeartbeatsWhileWaiting`, `mitm.TestDelegateHandler_ResolveDelegateRunsConcurrentlyWithWriteReply`. No structural risk of a delegated reply interleaving with the front CLI's own separate output: every inbound connection gets its own goroutine and its own `http.ResponseWriter` (stdlib `net/http.Server`), so two different CLI processes' streams are never on the same writer, and within one CLI the delegate flag fully substitutes that turn — the front's own model never sees the message at all. |
