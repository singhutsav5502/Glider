...
| Delegate replies over cursor-agent's own protocol can time out on a slow delegate target | **Confirmed live 2026-07-29, real, root-caused, NOT yet fixed.** Everything upstream of this now genuinely works end-to-end, confirmed live: `cursor-agent -p "... /agy"` under Glider's own transparent redirector (no proxy env vars) reaches `"mitm delegate: routing to vendor" front=cursor-agent vendor=agy` — the actual delegate dispatch fires correctly. But the reply write then fails with `http2: stream closed`: `DelegateHandler` computes the full reply text via `vendors.ResolveDelegate` (which runs the target vendor's CLI headless — for `agy`, this can take many seconds) *before* ever touching the response writer, so cursor-agent's own HTTP/2 client sits with zero bytes received for the entire delegate run and gives up. `internal/cursorrpc.WriteRunSSEResponse` (used by `cursorOriginAdapter.WriteReply`) already has an established heartbeat-first pattern for exactly this situation, but `WriteReply` is only ever called once, after the reply text is already fully known — the heartbeat never gets a chance to go out early. Needs `DelegateHandler` to start replying (or at least keep the stream alive) *before* the headless call finishes, not after — an architectural change to the delegate-reply flow, not a small fix, so deliberately not implemented without design sign-off first (see `planning/permission_relay_design.md` and the NGL docs for the existing shape this would need to fit into). |
