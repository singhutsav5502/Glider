...
## Transparent interception (Windows: WinDivert — Linux: iptables)
...
The redirector narrows by process name using the same vendor registry the delegation route reads (`internal/vendors`, `configs/vendor_candidates.yaml`) — run discovery from the dashboard's **Vendors** tab first, or transparent mode has no process-based narrowing and only the IP/port filter applies. Implementation: `internal/mitm/redirector_windows.go` on Windows, and `internal/mitm/redirector_linux.go` on Linux — the Linux side uses an `iptables` REDIRECT rule in the nat OUTPUT chain plus `SO_ORIGINAL_DST` to recover the true destination, and shells out via `sudo -n iptables`. `internal/mitm/redirector_other.go` is a stub that returns a clear "not implemented" error; macOS lands there today, and `windivert_dll_path` is a Windows-only setting. Design notes: [`planning/transparent_redirector_design.md`](../planning/transparent_redirector_design.md).
...
| Transparent interception scope | Windows (WinDivert) and Linux (iptables); macOS is a stub. Live-verified end-to-end for Claude Code, not yet for cursor-agent/agy in that same zero-cooperation mode |
