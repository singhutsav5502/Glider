...
**Global singleton risk:** `Default()` / `SetDefault()` make package-level side effects hard to test. Replace with explicit injection (already mostly done via interfaces; remove the global).
...
**Remaining (DIP):** replace concrete `*loop.Manager`, `*swarm.Runner` fields with narrow interfaces (only the methods each handler group uses).
