...
| **Multi-agent / swarms** | **~65%** | FanOut+Merge+CritiqueMerge; sample Starlark + templates; not Slate weave |
| **Loop engineering** | **REMOVED 2026-07-25** | `internal/loop` deleted whole in the v1 CLI-interop strip-down; see `planning/Depreceated/loop_engineering.md` for history |
...
|-----------|-------------|--------|
| Babysit / CI wake | Wake → local fix | Not built |
| Swarm wave | FanOut + Episode weave | Stub |
| ~~Loop Engineering cycle~~ | Planner→Actor→Critic + memory/router + eval | **REMOVED 2026-07-25** — see `planning/Depreceated/loop_engineering.md` |
