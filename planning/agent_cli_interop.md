...
>
> Related: [MITM_NETWORK.md](../docs/MITM_NETWORK.md) (Cursor IDE protobuf path — the prior art this extends) · `internal/cursorrpc/toolcall_map.go` (existing Cursor wire↔builtin map) · `internal/tools/registry.go` (Glider's canonical builtin schema) · [cursor_agent_protocol_interception.md](./cursor_agent_protocol_interception.md) · **[native_glider_orchestration.md](./native_glider_orchestration.md)** — the product vision this feeds: naming this design "NGL" and using it to let one CLI's session transparently delegate to headless sessions of the others
...
One file per vendor, versioned against an observed CLI build string, checked into the repo like any other config, hot-reloadable the same way `routing.turn_family_ttl` already is (`docs/MITM_NETWORK.md` §Config knobs) — or via the reload pattern already taking shape in `internal/backend/reload.go` / `internal/swarm/hotswap.go`, if that turns out to be the natural place to hang it; not a hard dependency, just the closest existing precedent for "swap config without restart" in this codebase. A new CLI is a new YAML file, not a new Go package. A renamed tool in a point release is a one-line diff to the pack, not a redeploy.
...
| cursor-agent exact completion-plane RPC name / raw bytes | **Resolved as a negative result, which is itself the answer.** A second, mutex-fixed capture pass (27 clean, non-interleaved requests) proved `--endpoint`/`CURSOR_API_ENDPOINT` **does not cover the completion plane at all** — only Dashboard/Config/Models/Analytics. The completion call goes to a fixed host outside that override, consistent with `docs/MITM_NETWORK.md`'s already-documented `api2`–`api5.cursor.sh` cluster for the IDE. Getting its raw bytes requires real TLS MITM (CA trust) — correctly stayed out of scope per this session's decision not to touch the OS trust store without explicit sign-off. `stream-json` remains the practical ceiling for that plane; this is an architectural fact now, not an open question. |
