...
`{{prompt}}`, `{{session_id}}`, and `{{cwd}}` are substituted at exec time — template *data*, not vendor-specific Go code. A message can select a non-default template with a `:name` suffix on the delegate flag (`/agy:interactive …`), parsed by `vendors.ParseDelegateCommand`.
