...
// BidiAppendInspect gives a view of the envelope and of the content of a
// request body for BidiService/BidiAppend, on a current version of Cursor
// (api2). It gives the best result that it can.
//
// This is not a full decode of the schema. Passthrough to the origin is still
// necessary, until a person verifies a codec for the response.
//
// Live dumps from Cursor 3.12, in July 2026, show an outer shape that repeats:
//
//	field 1, length-delimited — the append data. It is frequently the
//	    ASCII-hex form of an inner proto message.
//	field 2, length-delimited — a nested message with { field 1 = the UUID
//	    of the request }.
//	field 3, varint — the sequence, or the index of the append.
//
// After the code removes the hex form, or with the raw form, the number of the
// top-level field inside appears to identify the type of the message. These are
// hypotheses. Refer to RoleGuess and to the document with the findings.
