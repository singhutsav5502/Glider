...
// peekClientHelloSNI takes the SNI host name out of a raw TLS ClientHello
// record. It does not consume the record.
//
// The caller must get buf with bufio.Reader.Peek. Therefore the code can give
// the same reader to mitmSession or to blindTunnel after this, and no code
// consumed any byte.
//
// This is the same method that handleCONNECT uses. That function gives its br
// with no change, after it reads the CONNECT line from the wire, and not the
// bytes of TLS.
//
// Only the transparent entry point needs this function. The CONNECT path gets
// its host name from the request line with no work, and it never needs
// this.
