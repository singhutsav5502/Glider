...
func (i *Interceptor) handleAgentRPC(w http.ResponseWriter, r *http.Request, host, path string, start time.Time) (bool, error) {
	...
	// agent.v1.AgentService/Run is a true RPC that streams in two directions. This
	// applies to that call only, and not to RunSSE, BidiAppend or StreamChat*.
	//
	// Its true client keeps the send side of its request stream open for
	// approximately 30 seconds, and it sends envelopes that keep the connection
	// alive. Therefore an io.ReadAll(r.Body) here that blocks uses that full
	// period, and it gives no message, before this handler or the passthrough to
	// the origin below can answer. That period is long enough for the client to
	// stop and to reset the stream first.
	//
	// This is a true incident, and a live test confirmed it. Refer to the comment
	// on cursorrpc.ReadFirstEnvelope. A person corrected DelegateHandler in the
	// same way on 2026-07-29, and that handler operates before this handler in the
	// chain. This code corrects the same problem for the plain passthrough path,
	// which has no delegate and which this handler covers.
	//
	// The prompt of the person is always the first envelope on this shape of call.
	// Therefore a read of that envelope alone loses nothing. Each other shape of
	// AgentRPC keeps the plain io.ReadAll, because those shapes do not stream in
	// two directions.
	...
	// Use io.MultiReader with the first envelope from the buffer and then r.Body.
	// This returned on 2026-07-30. Refer to the same correction in
	// DelegateHandler.TryHandle and to its comment for the full sequence of changes
	// in this investigation.
	//
	// To use only the first envelope stopped the plain passthrough, which has no
	// delegate. The true origin at cursor.sh receives only a short request that
	// the code closed before its end, and not the full exchange in two directions
	// that a true client sends. Then that origin appears to never complete a
	// response.
	//
	// A person used a MultiReader, then removed it because a block of approximately
	// 30 seconds returned, and then found the true cause: passthroughHTTPS used the
	// context of this request. Therefore the early self-cancellation of the true
	// client stopped the full outbound relay.
	//
	// passthroughHTTPS now uses an independent context for the outbound part.
	// Therefore it is safe to relay the live stream here again. A block on the next
	// keepalive of the client only continues at the speed of the client, and the
	// code does not stop it early.
	//
	// The code does NOT set Content-Length for the Run path, and this is on
	// purpose. The total size of the body that the code makes again is not known at
	// the start. This agrees with the frames of a true request that streams in two
	// directions over HTTP/2: the DATA frames give the size, until END_STREAM, and
	// a header does not.
...
func (i *Interceptor) maybeBidiExtractDecide(r *http.Request, body []byte, host, path, ct string, log *slog.Logger) {
	...
	// The protection for composer_wrapup_origin: each wrap-up pack of Chrome and
	// each title pack always calls ArmOrigin. It does this also when the TTL and
	// the grace period of StickyCloud are complete. A dump showed the leak: "Friday
	// stock market close" with user_visible_high_level_summary went to Small
	// Context Local.
...
func (i *Interceptor) tryRunSSEFulfill(w http.ResponseWriter, r *http.Request, body []byte, host, path string, start time.Time, log *slog.Logger) (bool, error) {
	...
	// This is a true incident, and a live test confirmed it on 2026-07-31.
	//
	// This traffic can come from a subprocess of cursor-agent that Glider started
	// itself. That is a delegate call of Path A, from vendors.RunWithOptions, and
	// it is not a true session of a front CLI.
	//
	// The AgentService/Run calls of that subprocess agree with exactly the same
	// filter for transparent interception as a session that a person drives.
	// Therefore they come to this same code of Path B. This code then waits
	// defaultRunSSEFulfillWait, which is 800ms, for a decision about local
	// fulfillment. That decision NEVER arrives, because no code in the delegate
	// flow of Path A makes one.
	//
	// One delegate task with many steps makes some such RunSSE calls, and each one
	// pays that wait of 800ms with no result.
