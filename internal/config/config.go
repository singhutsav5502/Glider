...
type ContextConfig struct {
	...
	// Summary controls compaction of the per-workspace continuity record.
	Summary SummaryConfig `yaml:"summary,omitempty" json:"summary,omitempty"`
	// Background bounds the orienting context handed to each delegate.
	Background BackgroundConfig `yaml:"background,omitempty" json:"background,omitempty"`
}

// BackgroundConfig limits the "Earlier in the calling session" block that goes
// with each delegate call.
//
// This is a decision about cost, and it is not a decision about correctness.
// The block goes with EVERY delegation. Therefore a large budget gives better
// orientation, and it costs more tokens and more time each time. A person who
// delegates to a small local model and a person who delegates to a large cloud
// model need different values.
type BackgroundConfig struct {
	// TokenBudget is the approximate ceiling on the whole block. 0 → 20000,
	// the scale Codex CLI keeps verbatim beside its summary. It is a ceiling,
	// not a target: a short session produces a short block.
	TokenBudget int `yaml:"token_budget,omitempty" json:"token_budget,omitempty"`
	// MaxEntries limits the count of entries in the block. A value of 0 gives 20.
	// Keep it in proportion with TokenBudget. A value that is too small applies
	// before the budget can apply, and that condition made the old fixed value of 6
	// the true limit.
	MaxEntries int `yaml:"max_entries,omitempty" json:"max_entries,omitempty"`
	// MaxEntryChars limits one entry, and it removes the middle. A value of 0
	// gives 4000, which is approximately 1000 tokens. That is sufficient for a long
	// instruction or for a stack trace of a usual size. It also stops one very
	// large paste from using the full budget.
	MaxEntryChars int `yaml:"max_entry_chars,omitempty" json:"max_entry_chars,omitempty"`
}

// SummaryConfig selects who makes the summary of a long continuity record.
//
// Compaction always occurs. This setting only decides if a model makes the
// summary, or if the method with no model makes it. The work occurs in the
// background, and never on the clock of a request. That is the cause that
// permits an option as slow as a full start of a CLI.
type SummaryConfig struct {
	// Enabled turns model-backed summarization on. False still compacts,
	// using the deterministic digest.
	Enabled bool `yaml:"enabled,omitempty" json:"enabled,omitempty"`
	// Chain is the sequence of preference. The code tries each value until one
	// succeeds. The values:
	//
	//   "origin"  run the origin CLI with no console. This spends the
	//             subscription that the user already has, and it needs the
	//             credentials of no other person.
	//   "cloud"   a backend with your own key.
	//   "local"   a local model.
	//   "none"    stop here.
	//
	// An empty value gives origin, then cloud, then local.
	//
	// "origin" is first on purpose. The user already pays for a CLI, and that
	// CLI can do the same work. Therefore the bookkeeping of Glider must not
	// spend the API credits of that user.
	//
	// This runs that CLI as a usual delegate with no console. **It never uses
	// again the credentials that Glider observed in intercepted traffic.** §8 of
	// the high-level design gives that pattern as a risk with the terms of
	// service. Do not change this.
	Chain []string `yaml:"chain,omitempty" json:"chain,omitempty"`
	// CloudBackend/CloudModel name the backend used when the chain reaches
	// "cloud". Empty → the configured default cloud backend.
	...
	// LocalModel names the model used when the chain reaches "local".
	// Empty → routing.default_local_model.
	LocalModel string `yaml:"local_model,omitempty" json:"local_model,omitempty"`
	// OriginVendor selects which CLI answers for "origin". An empty value gives
	// the front CLI that owns the workspace that the code compacts.
	OriginVendor string `yaml:"origin_vendor,omitempty" json:"origin_vendor,omitempty"`
}
...
type MITMConfig struct {
	...
	// Transparent enables the interception of packets at the level of the
	// operating system. Refer to planning/transparent_redirector_design.md. This
	// is the primary method for each vendor that does not accept a proxy setting
	// or an environment variable for the base URL. That design document also
	// makes it the default for the vendors that do accept those. Windows only
	// for now (WinDivert); other OSes return a clear "not implemented" error
	// from Proxy.Start rather than silently no-op'ing.
	...
	// no-op even if true, since there is nothing to load.
...
type RoutingConfig struct {
	...
	// DefaultLocalModel gives the model when Default=local and an always rule
	// needs one.
...
type TransformConfig struct {
	...
	// LocalEpisodeCount adds a maximum of N earlier episodes, in a compressed
	// form, as a system message before a fulfillment by a local model. A value
	// of 0 disables this. When the yaml file does not set it, the default is 3.
	// Keeps locals off TipTap mega-dumps.
