...
// The code reads DefaultEnvFiles in sequence. A later file has control over the
// values of an earlier file.
//
// The code never writes over a key that the environment of the process already
// has. The shell, or the CI system, has control.
var DefaultEnvFiles = []string{".env", ".env.local"}

// LoadDotEnvFiles reads lines with the form KEY=VALUE from paths. It passes a
// file that is absent.
