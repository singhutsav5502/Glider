...
// TestParseCursorToolCall_PopulatesID is the regression test for the true
// defect that this pass found. The earlier version of ParseCursorToolCall never
// put a value in ToolCall.ID. It examined only the inner object
// tool_call.<name>, and it never examined call_id or toolCallId in the outer
// envelope.
...
// TestParseCursorAgentTurn_SystemInitAndResultCarryNoParts examines the two
// types of event that hold data about the session and the protocol, and which
// hold no content of a turn. The code must keep Raw, and Parts must stay empty.
// The code must not put incorrect data in Parts.
...
// TestCursorGlobResult_MatchesRealShape replaces an earlier guess that no
// person verified. That guess was {filenames, durationMs, numFiles, truncated,
// totalMatches, countIsComplete}, and none of those names are true.
//
// This test uses the shape that a person captured live: pattern, path, files,
// totalFiles, clientTruncated and ripgrepTruncated.
...
// TestCursorGrepArgs_MatchesRealShape covers the args struct that did not
