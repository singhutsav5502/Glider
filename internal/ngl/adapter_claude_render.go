...
// claudeDelegateRenderer cleans up Claude Code's headless "-p --output-format
// stream-json" output — raw NDJSON (system/init, every assistant delta,
// every tool_use/tool_result, the works) — down to just the final answer.
// The terminal "result" line's own Result field is already Claude's own
// synthesized final text (ClaudeResultEvent, adapter_claude.go) — this
// renderer doesn't re-derive an answer from the transcript, it just finds
// the one line that already IS the answer.
...
func (claudeDelegateRenderer) Render(raw []byte) (string, bool) {
	...
	if text == "" {
		// A result event with no text (seen on some error shapes) has
		// nothing clean to show — the raw transcript is more useful than
		// an empty reply.
		return "", false
	}
	return text, true
