...
// ToolsUnsupportedError reports that a backend refused tools[]. This is usual
// on an Ollama model that cannot call a tool. FallbackChain then tries the
// next step (typically BYOK cloud). See planning/routing_and_context.md.
...
type CompletionRequest struct {
	...
	// Tools / ToolChoice are first-class Path A fields (OpenAI or
	// Anthropic-normalized). Stored as RawMessage so schemas pass through to
	// Ollama/vLLM/OpenAI unchanged. The code parses the tool_calls of a stream
	// into CompletionChunk.ToolCalls. It then sends them again on the SSE of the
	// gateway, through the M2 bridge. See planning/routing_and_context.md.
