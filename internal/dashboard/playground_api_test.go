...
// TestPlaygroundParse_HeadlessVendorSortsFirst pins the order of the vendor
// list that the page uses to build its examples.
//
// The first lesson on that page is "send a task, and get the answer back". The
// page takes the FIRST name in this list for its example. A vendor with an
// interactive default opens a window and returns nothing, so an example built
// from it teaches the opposite of the lesson.
//
// The registry below puts the interactive vendor first on purpose. A pass
// means the sort moved the headless one in front of it, and that the test
// reads the mode of the template rather than the name of the vendor.
func TestPlaygroundParse_HeadlessVendorSortsFirst(t *testing.T) {
	isolateVendorRegistry(t)
	writeTestRegistry(t, vendors.Registry{Vendors: []vendors.Vendor{
		{Name: "opens-a-window", Binary: "a", Path: filepath.Join("fake", "a.exe"), PrintFlag: "-p", Enabled: true,
			Templates: []vendors.CommandTemplate{{Name: "default", Args: []string{"{{prompt}}"}, Mode: "interactive"}}},
		{Name: "returns-an-answer", Binary: "b", Path: filepath.Join("fake", "b.exe"), PrintFlag: "-p", Enabled: true,
			Templates: []vendors.CommandTemplate{{Name: "default", Args: []string{"-p", "{{prompt}}"}, Mode: "headless"}}},
	}})
	ts, _, _, _ := setupDash(t)

	out := playgroundParse(t, ts.URL, "hello")
	raw, ok := out["vendors"].([]any)
	if !ok || len(raw) != 2 {
		t.Fatalf("expected two vendors, got %+v", out["vendors"])
	}
	if raw[0] != "returns-an-answer" {
		t.Fatalf("the headless vendor must come first, so the example returns an answer; got %v", raw)
	}
}
