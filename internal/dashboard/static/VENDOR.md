...
- Used by: `app.js`'s graph rendering helpers (`stageCy`/`swarmCy`) — vendored for the dashboard's node/edge visualizations (nodes, edges, pan/zoom, drag)
- Why this library: mature vanilla JS graph editor (no React), MIT, editable nodes/edges, large-canvas pan/zoom, small enough to vendor; lighter than JointJS / Rete / xyflow for this dashboard.
...
- Used by: drag node-to-node edge drawing/reconnection on Cytoscape canvases (v4 uses draw mode)
